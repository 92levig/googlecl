...
class BaseServiceCL(gdata.service.GDataService):
  ...
  def RequestAccess(self):
    ...
    message_format = 'Please log in and grant access via your browser %s' +\
                     ' then hit enter.  This only needs to be done once' \
                     ' for each service and account pair you use with this' \
                     ' program.'
    if browser:
