...
function check_tasks_number {
    ...
    should_be \
        "python google.py calendar list --date $task_date -u $auth_username" \
        $1 \
        2 \
        "calendar entry" \
        "export PYTHONPATH=\"$gdata_directory/gdata-2.0.1/lib/python\" && python ../src/google.py calendar delete --date $task_date --title \".*\" -u $auth_username --yes"
...
do
  ...
  if [[ $auth_executed == "0" ]]; then
    auth_executed=1 
    python google.py calendar list --date $task_date --force-auth -u $auth_username
  fi
  ...
  # Deleting the task
  python google.py calendar delete --date $task_date --title "$task_title" -u $auth_username --yes
